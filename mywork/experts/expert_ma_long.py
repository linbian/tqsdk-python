...
SYMBOL = "SHFE.rb2005"
acc = TqAccount("快期模拟", "linbian@yeah.net", "17FH@Qiy305wjw)Gl59MmEjs")
api = TqApi(acc,backtest=TqBacktest(start_dt=date(2019, 5, 1), end_dt=date(2020, 3, 1)))
print("策略开始运行")
...
while True:
    ...
    if api.is_changing(klines.iloc[-1], "datetime"):  # 产生新k线:重新计算各指标
        ...
        if position.pos_long < 0 and (conditionShortClose1 or conditionShortClose2 or conditionShortClose3):
            print("平空仓")
        ...
        if position.pos_long < 0 and (conditionShortModify1 or conditionShortModify2 or conditionShortModify3):
            print("调空仓")